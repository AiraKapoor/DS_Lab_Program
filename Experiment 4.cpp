/*Q:Write a menu driven program that implements the following operations on a    doubly and Circular linked list :
...
void linked_list::deletion()
{
    ...
    node *temp,*p;
    p=h;
    if(h->info==ele)   //Deletion in beginning
    {
        h=h->next;
        tail->next=h;
        h->prev=tail;
